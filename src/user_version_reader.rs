...
impl UserVersionReader {
    pub fn into_user_version(self, config: &NvcConfig) -> Option<UserVersion> {
        match self {
...
mod tests {
    ...
    fn test_file_pathbuf_to_version() {
        ...
        let user_version =
            UserVersionReader::Path(pathbuf).into_user_version(&NvcConfig::default());
    ...
    fn test_direct_to_version() {
        let user_version = UserVersionReader::Direct(UserVersion::OnlyMajor(14))
            .into_user_version(&NvcConfig::default());
        assert_eq!(user_version, Some(UserVersion::OnlyMajor(14)));
    }
